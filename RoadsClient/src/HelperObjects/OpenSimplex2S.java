...
public class OpenSimplex2S {
	 ...
	 * Might be better for a 2D sandbox style game, where Y is vertical. Probably
	 * slightly less optimal for heightmaps or continent maps, unless your map is
	 * centered around an equator. It's a slight difference, but the option is here
	 * to make it easy.
	 */
	 ...
	 * should always be the "different" coordinate in whatever your use case is. If
	 * Y is vertical in world coordinates, call noise3_ImproveXZ(x, z, Y) or use
	 * noise3_XZBeforeY. If Z is vertical in world coordinates, call
	 * noise3_ImproveXZ(x, y, Z). For a time varied animation, call
	 * noise3_ImproveXY(x, y, T).
	 */
	 ...
	 * should always be the "different" coordinate in whatever your use case is. If
	 * Y is vertical in world coordinates, call noise3_ImproveXZ(x, Y, z). If Z is
	 * vertical in world coordinates, call noise3_ImproveXZ(x, Z, y) or use
	 * noise3_ImproveXY. For a time varied animation, call noise3_ImproveXZ(x, T, y)
	 * or use noise3_ImproveXY.
	 */
	 ...
	 * noise3_ImproveXY or noise3_ImproveXZ instead, wherever appropriate. They have
	 * less diagonal bias. This function's best use is as a fallback.
	 */
	...
	private static float noise3_UnrotatedBase(long seed, double xr, double yr, double zr) {
		...
		float value = (a0 * a0) * (a0 * a0) * grad(seed, xrbp + (xNMask & PRIME_X), yrbp + (yNMask & PRIME_Y),
				zrbp + (zNMask & PRIME_Z), x0, y0, z0);
		...
		if (a2 > 0) {
			...
			float y2 = y0;
			float z2 = z0;
			value += (a2 * a2) * (a2 * a2) * grad(seed, xrbp + (~xNMask & PRIME_X), yrbp + (yNMask & PRIME_Y),
					zrbp + (zNMask & PRIME_Z), x2, y2, z2);
		} else {
			...
			if (a3 > 0) {
				...
				value += (a3 * a3) * (a3 * a3) * grad(seed, xrbp + (xNMask & PRIME_X), yrbp + (~yNMask & PRIME_Y),
						zrbp + (~zNMask & PRIME_Z), x3, y3, z3);
			}
			...
			if (a4 > 0) {
				...
				float z4 = z1;
				value += (a4 * a4) * (a4 * a4)
						* grad(seed2, xrbp + (xNMask & (PRIME_X * 2)), yrbp + PRIME_Y, zrbp + PRIME_Z, x4, y4, z4);
				skip5 = true;
			}
		}
		...
		if (a6 > 0) {
			...
			float z6 = z0;
			value += (a6 * a6) * (a6 * a6) * grad(seed, xrbp + (xNMask & PRIME_X), yrbp + (~yNMask & PRIME_Y),
					zrbp + (zNMask & PRIME_Z), x6, y6, z6);
		} else {
			...
			if (a7 > 0) {
				...
				value += (a7 * a7) * (a7 * a7) * grad(seed, xrbp + (~xNMask & PRIME_X), yrbp + (yNMask & PRIME_Y),
						zrbp + (~zNMask & PRIME_Z), x7, y7, z7);
			}
			...
			if (a8 > 0) {
				...
				float z8 = z1;
				value += (a8 * a8) * (a8 * a8)
						* grad(seed2, xrbp + PRIME_X, yrbp + (yNMask & (PRIME_Y << 1)), zrbp + PRIME_Z, x8, y8, z8);
				skip9 = true;
			}
		}
		...
		if (aA > 0) {
			...
			value += (aA * aA) * (aA * aA) * grad(seed, xrbp + (xNMask & PRIME_X), yrbp + (yNMask & PRIME_Y),
					zrbp + (~zNMask & PRIME_Z), xA, yA, zA);
		} else {
			...
			if (aB > 0) {
				...
				float zB = z0;
				value += (aB * aB) * (aB * aB) * grad(seed, xrbp + (~xNMask & PRIME_X), yrbp + (~yNMask & PRIME_Y),
						zrbp + (zNMask & PRIME_Z), xB, yB, zB);
			}
			...
			if (aC > 0) {
				...
				value += (aC * aC) * (aC * aC)
						* grad(seed2, xrbp + PRIME_X, yrbp + PRIME_Y, zrbp + (zNMask & (PRIME_Z << 1)), xC, yC, zC);
				skipD = true;
			}
		}
		...
		if (!skip5) {
			...
			if (a5 > 0) {
				...
				value += (a5 * a5) * (a5 * a5) * grad(seed2, xrbp + PRIME_X, yrbp + (yNMask & (PRIME_Y << 1)),
						zrbp + (zNMask & (PRIME_Z << 1)), x5, y5, z5);
			}
		}
		...
		if (!skip9) {
			...
			if (a9 > 0) {
				...
				value += (a9 * a9) * (a9 * a9) * grad(seed2, xrbp + (xNMask & (PRIME_X * 2)), yrbp + PRIME_Y,
						zrbp + (zNMask & (PRIME_Z << 1)), x9, y9, z9);
			}
		}
		...
		if (!skipD) {
			...
			if (aD > 0) {
				...
				float zD = z1;
				value += (aD * aD) * (aD * aD) * grad(seed2, xrbp + (xNMask & (PRIME_X << 1)),
						yrbp + (yNMask & (PRIME_Y << 1)), zrbp + PRIME_Z, xD, yD, zD);
			}
		}
	...
	/**
	 * 4D SuperSimplex noise, with XYZ oriented like noise3_ImproveXY and W for an
	...
	/**
	 * 4D SuperSimplex noise, with XYZ oriented like noise3_ImproveXZ and W for an
	 * extra degree of freedom. W repeats eventually. Recommended for time-varied
	...
	/**
	 * 4D SuperSimplex noise, with XYZ oriented like noise3_Fallback and W for an
	 * extra degree of freedom. W repeats eventually. Recommended for time-varied
	 ...
	 * competes performance-wise with optimized 2014 OpenSimplex.
	 */
	private static float noise4_UnskewedBase(long seed, double xs, double ys, double zs, double ws) {
		...
		int index = ((fastFloor(xs * 4) & 3) << 0) | ((fastFloor(ys * 4) & 3) << 2) | ((fastFloor(zs * 4) & 3) << 4)
				| ((fastFloor(ws * 4) & 3) << 6);
